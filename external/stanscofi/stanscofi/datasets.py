...
#         "ignore",
#         message=".*he 'nopython' keyword argument was not supplied to the 'numba.jit' decorator.*",
#     )
#     import umap
...
def generate_dummy_dataset(
    npositive, nnegative, nfeatures, mean, std, random_state=12454
):
    """
...
class Dataset:
    ...
    def __init__(
        ...
        if self.same_item_user_features:
            features = list(
                set(self.item_features).intersection(set(self.user_features))
            )
    ...
    def summary(self, sep="-" * 70):
        ...
        restricted_ratings = self.ratings.toarray()[
            np.abs(self.ratings).sum(axis=1) > 0, :
        ]
        restricted_ratings = restricted_ratings[
            :, np.abs(restricted_ratings).sum(axis=0) > 0
        ]
        print(
            "Including %d drugs and %d diseases involved in at least one positive/negative rating"
            % restricted_ratings.shape
        )
        print(
            "%d positive, %d negative, %d unlabeled (including %d unavailable) drug-disease ratings"
            % (
                (self.ratings == 1).sum(),
                (self.ratings == -1).sum(),
                np.prod(self.ratings.shape) - self.ratings.nnz,
                np.prod(self.folds.shape) - self.folds.nnz,
            )
        )
        print(
            "Sparsity: %.2f percent (on drugs/diseases with at least one known rating %.2f)"
            % ((self.ratings != 0).mean() * 100, (restricted_ratings != 0).mean() * 100)
        )
        print(sep[: len(sep) // 2])
        ...
        return (
            self.nitems,
            self.nusers,
            restricted_ratings.shape[0],
            restricted_ratings.shape[1],
            (self.ratings == 1).sum(),
            (self.ratings == -1).sum(),
            np.prod(self.ratings.shape) - self.ratings.nnz,
            np.prod(self.folds.shape) - self.folds.nnz,
            (self.ratings != 0).mean() * 100,
            (restricted_ratings != 0).mean() * 100,
            self.items.shape[0],
            np.isnan(self.items.toarray()).mean() * 100,
            self.users.shape[0],
            np.isnan(self.users.toarray()).mean() * 100,
    ...
    def visualize(
        ...
        if self.users.shape[0] == 0 or self.items.shape[0] == 0:
            ...
            return
        assert predictions is None or (
            (
                (predictions.toarray() == 1)
                | (predictions.toarray() == -1)
                | (predictions.toarray() == 0)
            ).all()
        )
        if X is None and y is None:
            if verbose:
                print(
                    "<datasets.visualize> Imputation of missing values by average row-value, standard scaling"
                )
            subselect_size = max(2, min(int(5e7) // nvalues + 1, nvalues))
            subselect_size = min(
                subselect_size, min(self.users.shape[0], self.items.shape[0])
            )
            # Preprocessed (item, user) pair feature matrix and corresponding outcome vector
            X, y, _, _ = meanimputation_standardize(
                self, subset=subselect_size, inf=2, verbose=verbose
            )
            use_inputX = False
        else:
            predictions = None
        ...
        if predictions is None:
            ...
            all_pairs = np.array([
                [{-1: "r.", 1: "g.", 0: "y."}[k]] for i, j, k in markers.tolist()
            ])
            ...
            if not use_ratings:
                classes = dict(
                    zip(
                        [
                            (i, j)
                            for i, j in zip(
                                self.folds.row, self.folds.col, strict=False
                            )
                        ],
                        predictions.data,
                        strict=False,
                ...
                all_pairs = np.array([
                    [classes.get((i, j), 0)]
                    for i, j in markers[:, :2].astype(int).tolist()
                ])
                all_pairs = np.concatenate((markers, all_pairs), axis=1)
            else:
            ...
            if show_errors:
                values = np.array([
                    [
                        {0: "r", 1: "g", -1: "y"}[int(true == pred) - int(true == 0)]
                        + {-1: "v", 1: "+", 0: "."}[true]
                    ]
                    for [i, j, true, pred] in all_pairs.tolist()
                ])
                # Predicted ratings for all known pairs of items and users in the dataset
                # item i, user u, rating r, scatter style (color + marker shape)
                all_pairs = np.array(
                    np.concatenate((all_pairs[:, [0, 1, 3]], values), axis=1),
                    dtype=object,
                )
        ...
        if verbose:
            print(
                "<datasets.visualize> Reducing dimension and plotting matrix X of size %d x %d"
                % X.shape
            )
        dimred_args.update({
        ...
        if use_pca:
            ...
            dimred_args.update({
                "n_neighbors": max(
                    5, min(dimred_args["n_neighbors"], min(50, all_pairs.shape[0]))
                )
            })
            dimred_args.update({
                "min_dist": max(0.5, min(dimred_args.get("min_dist", 0.1), 0.001))
            })
            dimred_args.update({"metric": dimred_args.get("metric", "correlation")})
            if verbose:
                print(
                    "<datasets.visualize> n_neighbors = %d\tmin_dist = %.2f\tmetric = %s"
                    % (
                        dimred_args["n_neighbors"],
                        dimred_args["min_dist"],
                        dimred_args["metric"],
                    )
                )
            # with np.errstate(invalid="ignore"):  # for NaN or 0 variance matrices
        ...
        if X.shape[1] > 1:
            ...
            for mkr in np.unique(np.ravel(all_pairs[:, 3])).tolist():
                ...
                if (not withzeros) and (
                    ((predictions is None) and (mkr == "y."))
                    or ((predictions is not None) and (mkr[-1] == "."))
                ):
                    plt.scatter(
                        dimred_X[all_pairs_k, 0],
                        dimred_X[all_pairs_k, 1],
                        c="w",
                        marker=".",
                        zorder=0,
                        alpha=0,
                    ...
                    plt.scatter(
                        ...
                        alpha=alpha[mkr]
                        if (predictions is None)
                        else (0.8 if (not show_errors) else 1),
                    )
            if show_errors:
                handles = [
                    mlines.Line2D(
                        [],
                        [],
                        color={"r": "red", "g": "green", "y": "yellow"}[k[0]],
                        label={"r": "error   ", "g": "correct", "y": "unknown"}[k[0]]
                        + " "
                        + ({".": "[ 0]", "+": "[ 1]", "v": "[-1]"}[k[1]]),
                        marker=k[1] if (predictions is not None) else ".",
                        markersize=fontsize,
                    )
                    for k in np.unique(np.asarray(all_pairs[:, -1], dtype=str)).tolist()
                    if (withzeros or k[0] != "y")
                ]
            ...
            plt.ylabel(
                ("PC2 (" + str(int(var12[1])) + "%)" if (use_pca) else "Dim2")
                if (not np.isnan(var12[1]))
                else "C2",
                fontsize=fontsize,
            )
            plt.xlabel(
                ("PC1 (" + str(int(var12[0])) + "%)" if (use_pca) else "Dim1")
                if (not np.isnan(var12[0]))
                else "C1",
                fontsize=fontsize,
            )
            ...
            X_reshape_folds = coo_array(
                (X.ravel(), (self.folds.row, self.folds.col)), shape=self.folds.shape
            )
            ...
            handles = [
                mlines.Line2D([], [], color="black", marker=k)
                for k in ["+", "*"]
                if (k in annot)
            ]
            h.ax_heatmap.legend(
    ...
    def subset(self, folds, subset_name="subset"):
        ...
        users = pd.DataFrame(
            self.users.toarray(), index=self.user_features, columns=self.user_list
        )
        items = pd.DataFrame(
            self.items.toarray(), index=self.item_features, columns=self.item_list
        )
        subset = Dataset(
            ratings=ratings,
            users=users,
            items=items,
            same_item_user_features=self.same_item_user_features,
            name=subset_name,
        )
        return subset
