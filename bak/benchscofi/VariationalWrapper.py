...
from .implementations import VariationalInference
...
class VariationalWrapper(BasicModel):
    ...
    def preprocessing(self, dataset, is_training=True):
        ...
        for x in range(y.shape[0]):
            X[x, dataset.folds.col[x]] = 1.0
            X[x, dataset.folds.row[x] + N] = 1.0
        return (
            [torch.LongTensor(X), torch.LongTensor(y), N, M]
            if (is_training)
            else [torch.LongTensor(X)]
        )
    ...
    def model_fit(self, X, y, N, M):
        ...
        nb_samples = len(y)
        train_rmse = train_auc = train_map = 0.0
        losses = []
        all_preds = []
        ...
        self.model = VariationalInference.CF(
            self.EMBEDDING_SIZE, self.N_VARIATIONAL_SAMPLES, N=N, M=M, output="class"
        )
        # output="reg")
        self.optimizer = {
        ...
        for epoch in tqdm(range(self.N_EPOCHS)):
            ...
            if epoch % self.DISPLAY_EPOCH_EVERY == 0:
                ...
                print(
                    f"Epoch {epoch}: Elbo {np.mean(losses):.4f} "
                    + (
                ...
                print(
                    ...
                    self.model.alpha,
                    "std dev",
                    torch.sqrt(1 / nn.functional.softplus(self.model.alpha)),
                )
