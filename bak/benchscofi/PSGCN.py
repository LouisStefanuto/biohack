...
from .implementations import PSGCNImplementation
...
class PSGCN(BasicModel):
    ...
    def preprocessing(self, dataset, is_training=True):
        ...
        graphs = PSGCNImplementation.extract_subgraph(
            split_data_dict, self, k=0, is_training=is_training
        )  # ,filefolder=filefolder)
        if self.model is None:
            self.model = PSGCNImplementation.PSGCN(
                graphs,
        ...
        return (
            [graphs]
            if (is_training)
            else [graphs, dataset.folds.data.shape[0], vals.shape, dataset.folds]
        )

    def model_fit(self, train_graphs):
        PSGCNImplementation.train_multiple_epochs(
            train_graphs, train_graphs, self.model, self
        )
