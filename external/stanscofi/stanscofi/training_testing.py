...
from .validation import compute_metrics, metrics_list, plot_metrics
...
def random_simple_split(dataset, test_size, metric="cosine", random_state=1234):
    ...
    X = np.column_stack((
        dataset.folds.row,
        dataset.folds.col,
    ))  # indices for available ratings
    y = dataset.ratings.toarray()[
        dataset.folds.row, dataset.folds.col
    ].ravel()  # values for available ratings
    train_set, test_set, _, _ = train_test_split(
        X, y, test_size=test_size, random_state=random_state, shuffle=True, stratify=y
    )
    cv_folds = tuple([
        coo_array(
            (np.ones(x.shape[0]), (x[:, 0].ravel(), x[:, 1].ravel())),
...
def random_cv_split(dataset, cv_generator, metric="cosine"):
    ...
    dist_lst = [
        (
            np.ma.masked_equal(
                dist[np.unique(train_folds.row), :][:, np.unique(test_folds.row)],
                0.0,
                copy=True,
            ).min(),
            np.ma.masked_equal(
                dist[np.unique(train_folds.row), :][:, np.unique(train_folds.row)],
                0.0,
                copy=True,
            ).min(),
            np.ma.masked_equal(
                dist[np.unique(test_folds.row), :][:, np.unique(test_folds.row)],
                0.0,
                copy=True,
            ).min(),
        )
...
def weakly_correlated_split(
    ...
    while l_nc < u_nc:
        ...
        nratings_train = {
            drug_clusters[drug_clusters <= c].shape[0]: c
        ...
        if verbose:
            print(
                "<training_testing.traintest_validation_split> Find #clusters=%d in [%d, %d] (%d ~ %d?)"
                % (nc, l_nc, u_nc, select_clust, train_nset)
            )
    ...
    item_labels = (
        fcluster(Z, select_nc, criterion="maxclust", depth=2, R=None, monocrit=None)
        > cluster_size + 1
    ).astype(int) + 1
    ...
    dist_lst = (
        np.ma.masked_equal(
            dist[item_labels == 1, :][:, item_labels == 2], 0.0, copy=True
        ).min(),
        np.ma.masked_equal(
            dist[item_labels == 1, :][:, item_labels == 1], 0.0, copy=True
        ).min(),
        np.ma.masked_equal(
            dist[item_labels == 2, :][:, item_labels == 2], 0.0, copy=True
        ).min(),
    )
...
def cv_training(
    ...
    test_size=0.2,
    dist_type="cosine",
    cv_type=["random", "weakly_correlated"][0],
    early_stop=2,
    njobs=1,
    random_state=1234,
    show_plots=False,
    ...
    if cv_type == "random":
        cv_generator = StratifiedKFold(
            n_splits=nsplits, shuffle=True, random_state=random_state
        )
    ...
    def single_run(ncv, template, params, metric, tfolds, sfolds):
        ...
        metrics_train, _ = compute_metrics(
            scores_train, predictions_train, tdataset, metrics=[metric], verbose=verbose
        )
        ...
        metrics_test, plot_args = compute_metrics(
            scores_test,
            predictions_test,
            sdataset,
            metrics=[metric],
        ...
        if show_plots:
            plot_metrics(
                **plot_args,
                figsize=(10, 10),
                model_name="%s on %s (cv%d)\n"
                % (model.name, train_dataset.name, ncv + 1),
            )
        metric_train, metric_test = (
            metrics_train.loc[metric][metrics_train.columns[0]],
            metrics_test.loc[metric][metrics_test.columns[0]],
        )
        if verbose:
            print(
                "Crossvalidation step #%d/%d (train %s %f, test %s %f)"
                % (ncv + 1, nsplits, metric, metric_train, metric, metric_test)
            )
        return model, metric_train, metric_test

    results = parallel(
        delayed(single_run)(ncv, deepcopy(template), params, metric, tfolds, sfolds)
        for ncv, (tfolds, sfolds) in enumerate(cv_folds)
    )
    return {
        "models": [r[0] for r in results],
        "train_metric": [r[1] for r in results],
        "test_metric": [r[2] for r in results],
        "cv_folds": cv_folds,
    }
...
def grid_search(
    ...
    """
    best_params, best_model, best_test_metric, best_train_metric = (
        {},
        None,
        -float("inf"),
        0,
    ...
    for param in search_params:
        for param_val in search_params[param]:
            ...
            if verbose:
                print(
                    "<training_testing.grid_search> [%s=%s] best %s on Test %f (Train %f)"
                    % (
                        param,
                        str(param_val),
                        metric,
            ...
            if np.max(results["test_metric"]) > best_test_metric:
                ...
                best_model = deepcopy(
                    results["models"][np.argmax(results["test_metric"])]
                )
                best_test_metric = np.max(results["test_metric"])
                best_train_metric = results["train_metric"][
                    np.argmax(results["test_metric"])
                ]
    return (
        best_params,
        best_model,
        {"test_metric": best_test_metric, "train_metric": best_train_metric},
    )
