...
from .implementations import HANImplementation
...
class HAN(BasicModel):
    ...
    def preprocessing(self, dataset, inf=2, is_training=True):
        ...
        drug_drug = self.scalerS.fit_transform(
            dataset.items.T.toarray().copy(), subset=None
        )
        drug_drug = np.nan_to_num(drug_drug, nan=0)
        drug_drug = (
            drug_drug
            if (drug_drug.shape[0] == drug_drug.shape[1])
            else np.corrcoef(drug_drug)
        )
        if self.scalerP is None:
            self.scalerP = CustomScaler(posinf=inf, neginf=-inf)
        disease_disease = self.scalerP.fit_transform(
            dataset.users.T.toarray().copy(), subset=None
        )
        disease_disease = np.nan_to_num(disease_disease, nan=0)
        disease_disease = (
            disease_disease
            if (disease_disease.shape[0] == disease_disease.shape[1])
            else np.corrcoef(disease_disease)
        )
        ...
        data = torch.tensor(
            np.column_stack((dataset.folds.row, dataset.folds.col)).astype("int64")
        ).to(self.device)
        label = (
            torch.tensor(drug_disease[dataset.folds.row, dataset.folds.col].flatten())
            .float()
            .to(self.device)
        )
    ...
    def model_fit(self, g_train, train_data, train_label, shp):
        ...
        model = HANImplementation.HAN(
            in_feats=[feature["drug"].shape[1], feature["disease"].shape[1]],
            meta_paths=[
        ...
        criterion = torch.nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor(
                len(torch.where(train_label == 0)[0])
                / len(torch.where(train_label == 1)[0])
            )
        )
        print(
            f"BCE loss pos weight: {len(torch.where(train_label == 0)[0]) / len(torch.where(train_label == 1)[0]):.3f}"
        )

        train_idx = torch.tensor([
            (i.item() - 1) * shp[1] + j for (i, j) in train_data
        ]).to(self.device)
        for epoch in range(1, self.epoch + 1):
            ...
            if epoch % 100 == 0 or epoch == self.epoch - 1:
                model.eval()
                AUC, AUPR = HANImplementation.get_metrics_auc(
                    train_label.detach().cpu().numpy(),
                    pred_score.detach().cpu().numpy(),
                )
                print(
                    f"Epoch {epoch} Loss: {loss.item():.3f}; Train: AUC {AUC:.3f}, "
                    f"AUPR {AUPR:.3f}"
                )
        self.estimator = {
            "predictions": torch.sigmoid(model(g_train, feature)).detach().cpu().numpy()
        }
