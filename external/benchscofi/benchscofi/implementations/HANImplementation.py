...
class PGCN(nn.Module):
    ...
    def forward(self, g, feature):
        h = {
            "drug": self.drug_emb(feature["drug"]),
...
def remove_graph(g, test_drug_id, test_disease_id):
    ...
    edges_id = g.edge_ids(
        torch.tensor(test_drug_id), torch.tensor(test_disease_id), etype=etype
    )
    g = dgl.remove_edges(g, edges_id, etype=etype)
    etype = ("disease", "disease-drug", "drug")
    edges_id = g.edge_ids(
        torch.tensor(test_disease_id), torch.tensor(test_drug_id), etype=etype
    )
    g = dgl.remove_edges(g, edges_id, etype=etype)
    return g
...
def plot_result_auc(args, label, predict, auc):
    ...
    plt.plot(
        fpr, tpr, color="darkorange", lw=lw, label="ROC curve (area = %0.4f)" % auc
    )
...
def plot_result_aupr(args, label, predict, aupr):
    ...
    plt.plot(
        ...
        color="darkorange",
        lw=lw,
        label="AUPR Score (area = %0.4f)" % aupr,
    )
    plt.xlabel("Recall")
