...
class LRSSL(BasicModel):
    def __init__(self, params=None):
        ...
        cmd = (
            "wget -qO - 'https://raw.githubusercontent.com/LiangXujun/LRSSL/a16a75c028393e7256e3630bc8b7900026061f99/LRSSL.R' | sed -n '/###/q;p' > %s"
            % self.LRSSL_filepath
        )
        call(cmd, shell=True)
    ...
    def preprocessing(self, dataset, is_training=True, inf=2):
        ...
        if all([self.sep_feature in str(f) for f in dataset.item_features]):
            types_feature = [
                str(f).split(self.sep_feature)[0] for f in dataset.item_features
            ]
            X_lst = [
                S_[:, np.argwhere(np.array(types_feature) == tf)].T
                for tf in list(set(types_feature))
            ]
        ...
        S_lst = [
            x.T.dot(x) / np.sqrt(x.sum(axis=0).dot(x.sum(axis=0).T)) for x in X_lst
        ]
    ...
    def model_fit(self, X_lst, S_lst, Y):
        time_stamp = (
            calendar.timegm(current_GMT) + np.random.choice(range(int(1e8)), size=1)[0]
        )
        ...
        for s in S_lst:
            ...
            call(
                'R -q -e \'source("%s");S <- as.matrix(read.table("%s/s.csv", sep=" ", header=F));Sp <- get.knn.graph(S, %d);write.csv(S, "%s/s.csv", row.names=F)\' 2>&1 >/dev/null'
                % (self.LRSSL_filepath, filefolder, self.k, filefolder),
                shell=True,
            )
        ...
        assert all([
            s.shape[0] == Y.shape[0] and s.shape[1] == Y.shape[0] for s in S_lst
        ])
        assert all([
            l.shape[0] == Y.shape[0] and l.shape[1] == Y.shape[0] for l in L_lst
        ])
        assert all([
            x.shape[0] == Y.shape[0] and x.shape[1] == Y.shape[0] for x in X_lst
        ])
        call(
            'R -q -e \'source("%s");ml <- %d;mx <- %d;X_lst <- lapply(1:mx, function(i) as.matrix(read.table(paste0("%s/x_",i,".csv"), sep=" ", header=F)));L_lst <- lapply(1:ml, function(i) as.matrix(read.table(paste0("%s/l_",i,".csv"), sep=" ", header=F)));Y <- as.matrix(read.table("%s/y.csv", sep=" ", header=F));train.res <- lrssl(X_lst, L_lst, Y, mx, ml, %f, %f, %f, %d, %f);for(i in 1:mx){write.csv(train.res$Gs[[i]], paste0("%s/G_",i,".csv"), row.names=F)};write.csv(train.res$alpha, "%s/alpha.csv", row.names=F);write.csv(list(t=train.res$t,diff_G=train.res$diff.G), "%s/vals.csv", row.names=F);write.csv(train.res$F.mat, "%s/F_mat.csv", row.names=F)\' | grep \'\\[1\\]\''
            % (
                self.LRSSL_filepath,
                len(L_lst),
                len(X_lst),
        ...
        self.estimator = {
            "G": [
                np.loadtxt(
                    "%s/G_%d.csv" % (filefolder, i + 1), skiprows=1, delimiter=","
                )
            ...
            "diff_G": np.loadtxt("%s/vals.csv" % filefolder, skiprows=1, delimiter=",")[
                1
            ],
    ...
    def model_predict_proba(self, X_lst, shp):
        ...
        Y_lst = [
            self.estimator["alpha"][iy]
            * (y / np.tile(y.sum(axis=1), (y.shape[1], 1)).T)
            for iy, y in enumerate(Y_lst)
        ]
