...
class SCPMF(BasicModel):
    ...
    def model_fit(self, X_s, X_p, A_sp):
        ...
        call(
            "wget -qO %s/SCPMFDR.m 'https://raw.githubusercontent.com/luckymengmeng/SCPMF/master/SCPMFDR.m'"
            % filefolder,
            shell=True,
        )
        ...
        cmd = (
            "A_sp = csvread('A_sp.csv');X_s = csvread('X_s.csv');X_p = csvread('X_p.csv');recMatrix=SCPMFDR(A_sp,X_s,X_p,%d);csvwrite('recMatrix.csv', recMatrix);"
            % (self.r)
        )
