...
def ratings2matrix(ratings, user_col, item_col, rating_col):
    ...
    assert all([a in [-1, 0, 1] for a in np.unique(ratings[[rating_col]].values)])
    res_df = (
        ratings.pivot_table(index=item_col, columns=user_col, values=rating_col)
        .fillna(0)
        .astype(int)
    )
    return res_df
...
def load_dataset(model_name, save_folder="./", sep_feature="-"):
    ...
    if model_name == "LRSSL":
        ...
        fnames = {
            "A": "drug_dis_mat.txt",
            "P": "disease_similarity.txt",
            "S1": "drug_from_drugbank_without_ind_dommat.txt",
            "S2": "drug_pubchem_mat.txt",
            "S3": "drug_target_domain_mat.txt",
            "N4": "drug_target_go_mat.txt",
            "N5": "drug_without_ind_dommat_new.txt",
            "S6": "drug_without_ind_gomat_new.txt",
    ...
    if model_name in ["LRSSL2", "PREDICT_Gottlieb", "OrphanDrug"]:
        ...
        mmodel_name = (
            model_name
            if (model_name == "OrphanDrug")
            else ("PREDICT" if (model_name == "PREDICT_Gottlieb") else "LRSSL")
        )
        if not os.path.exists(dda_skf_dataset_path + mmodel_name + ".mat"):
            ...
            subprocess.call(
                " ".join([
                    "wget",
                    "-qO",
                    dda_skf_dataset_path + mmodel_name + ".mat",
                    url_dda_skf + mmodel_name + ".mat",
                ]),
                shell=True,
            )
        ...
        if mmodel_name == "LRSSL":
            ...
            S = pd.DataFrame(
                np.concatenate((S_chemical, S_sideeffects), axis=0),
                index=range(S_chemical.shape[0] + S_sideeffects.shape[0]),
                columns=["drug%d" % (i + 1) for i in range(S_chemical.shape[1])],
            )
            S.index = [
                ("chemical" if (iss < S_chemical.shape[0]) else "se")
                + sep_feature
                + ("drug%d" % s + 1)
                for iss, s in enumerate(list(S.index))
            ]
            P = ddt["lrsslsimmatdg"]
            P = pd.DataFrame(
                P,
                index=["disease%d" % (i + 1) for i in range(P.shape[0])],
                columns=["disease%d" % (i + 1) for i in range(P.shape[1])],
            )
            ...
            S = pd.DataFrame(
                np.concatenate((S_chemical, S_domain, S_GO), axis=0),
                index=range(S_chemical.shape[0] + S_domain.shape[0] + S_GO.shape[0]),
                columns=["drug%d" % (i + 1) for i in range(S_chemical.shape[1])],
            )
            S.index = [
                (
                    "chemical"
                    if (iss < S_chemical.shape[0])
                    else (
                        "domain"
                        if (iss < S_chemical.shape[0] + S_domain.shape[0])
                        else "go"
                    )
                )
                + sep_feature
    ...
    if model_name in ["TRANSCRIPT", "TRANSCRIPT_v1", "PREDICT", "PREDICT_v1"]:
        ...
        if not os.path.exists(path + fnames["A"]):
            if "TRANSCRIPT" in model_name:
                ...
                url_dataset = (
                    ...
                    else "https://zenodo.org/record/7982970/files/TRANSCRIPT_dataset_v1.0.0.zip"
                )
            else:
                print(
                    "Run the notebook or use the publicly available data (by default, download the latest public dataset)"
                )
                url_dataset = (
                    "https://zenodo.org/record/7983090/files/PREDICT_dataset_v2.0.0.zip"
                    if (model_name == "PREDICT")
                    else "https://zenodo.org/record/8087306/files/PREDICT_dataset_v1.0.0.zip"
                )
            subprocess.call(
                " ".join([
                    "wget",
                    "-qO",
                    save_folder + model_name + ".zip",
                    '"' + url_dataset + '"',
                ]),
                shell=True,
            )
            subprocess.call(
                " ".join(["unzip", "-qq", save_folder + model_name + ".zip"]),
                shell=True,
            )
            subprocess.call(
                " ".join([
                    ...
                    + "_dataset_v"
                    + ("2" if ("v1" not in model_name) else "1")
                    + ".0.0",
        ...
        if "TRANSCRIPT" in model_name:
            ...
            A, P, S = [
                pd.read_csv(path + fnames[k], engine="python", index_col=0)
                for k in ["A", "P", "S"]
            ]
        else:
            assert os.path.exists(path + fnames["A"])
            if os.path.exists(path + fnames["P"]) and os.path.exists(
                path + fnames["S"]
            ):
                A, P, S = [
                    pd.read_csv(path + fnames[k], engine="python", index_col=0)
                ...
                A = A[[a for a in A.columns if (a[-len(".1") :] != ".1")]]
                A.index = A.index.astype(str)
                P_phenotype = pd.read_csv(
                    path + "disease_phenotype_PREDICT_matrix.csv",
                    engine="python",
                    index_col=0,
                )
                P_semantic = pd.read_csv(
                    path + "disease_semantic_PREDICT_matrix.csv",
                    engine="python",
                    index_col=0,
                )
                S_se = pd.read_csv(
                    path + "se_PREDICT_matrix.csv", engine="python", index_col=0
                )
                S_signature = pd.read_csv(
                    path + "signature_PREDICT_matrix.csv", engine="python", index_col=0
                )
                P = P_phenotype.T.join(P_semantic.T, how="outer").T
                P.index = [
                    ("phenotype" if (iss < P_phenotype.shape[0]) else "semantic")
                    + sep_feature
                    + str(s)
                    for iss, s in enumerate(list(P.index))
                ]
                S = S_se.T.join(S_signature.T, how="outer").T
                if "v1" not in model_name:
                    S.index = [
                        ("se" if (iss < S_se.shape[0]) else "signature")
                        + sep_feature
                        + str(s)
                        for iss, s in enumerate(list(S.index))
                    ]
    ...
    if model_name in ["Gottlieb", "Fdataset"]:
        url_mbirw = (
            "https://raw.githubusercontent.com/bioinfomaticsCSU/MBiRW/master/Datasets/"
        )
        gottlieb_dataset_path = save_folder + "Gottlieb_dataset/MBiRW_files/"
        if not os.path.exists(gottlieb_dataset_path + "DiDrAMat"):
            subprocess.call(
                " ".join(["mkdir", "-p", gottlieb_dataset_path]), shell=True
            )
            for fname in [
                "DiDrAMat",
                "DiseaseSimMat",
        ...
        P, S = [
            pd.read_csv(gottlieb_dataset_path + fnames[k], sep=" ", header=None)
            for k in ["P", "S"]
        ]
        A = (
            pd.read_csv(gottlieb_dataset_path + fnames["A"], sep="\t", header=None)
            .iloc[:, :-1]
            .T
        )
        drug_names = pd.read_csv(
            gottlieb_dataset_path + names_fnames["drugs"], header=None
        ).T.values.tolist()
        drug_names = [x for x in drug_names]
        disease_names = pd.read_csv(
            gottlieb_dataset_path + names_fnames["diseases"], header=None
        ).T.values.tolist()
    ...
    if model_name == "indep":
        ...
        drug_names = pd.read_csv(
            indep_dataset_path + "R_Wrname", header=None
        ).T.values.tolist()[0]
        disease_names = pd.read_csv(
            indep_dataset_path + "R_Wdname", header=None
        ).T.values.tolist()[0]
        A.index = drug_names
        A.columns = disease_names
    ...
    if model_name == "Cdataset_Aonly":
        ...
        if not os.path.exists(cdataset_dataset_path + "DiDrMat"):
            subprocess.call(
                " ".join(["mkdir", "-p", cdataset_dataset_path]), shell=True
            )
        ...
        drug_names = pd.read_csv(
            cdataset_dataset_path + "DrugsName", header=None
        ).T.values.tolist()[0]
        disease_names = pd.read_csv(
            cdataset_dataset_path + "DiseasesName", header=None
        ).T.values.tolist()[0]
        A.index = drug_names
        A.columns = disease_names
        P = pd.DataFrame([], index=A.columns).T
            ...
            if not os.path.exists(drrs_dataset_path + model_name + "s.zip"):
                subprocess.call(
                    " ".join([
                        "wget",
                        "-qO",
                        drrs_dataset_path + model_name + "s.zip",
                        url_drrs + model_name + "s.zip",
                    ]),
                    shell=True,
                )
            subprocess.call(
        ...
        A, P, S = [
            pd.read_csv(path + fnames[k], sep="\t", header=None)
            for k in ["A", "P", "S"]
        ]
        A = A.T
...
def merge_ratings(rating_dfs, user_col, item_col, rating_col):
    ...
    for ratings in ratings_dfs:
        ...
        assert all([a in [-1, 0, 1] for a in np.unique(ratings[[rating_col]].values)])
    ratings = pd.concat(
        tuple([df.fillna(0)[[user_col, item_col, rating_col]] for df in rating_dfs]),
        axis=0,
    )
    ratings.index = [
        "--".join(list(ratings.iloc[idx][[user_col, item_col]].astype(str)))
        for idx in range(len(ratings.index))
    ]
    ratings[rating_col] = (
        ratings[[rating_col]]
        .groupby(level=0)
        .apply(lambda x: ((-1) ** ((x == -1).any().any())) * int((x != 0).any().any()))
        .astype(int)
    )
