...
from .implementations import LogisticMatrixFactorization
...
class LogisticMF(BasicModel):
    ...
    def model_predict_proba(self, X):
        ...
        fx += np.tile(
            self.estimator.user_biases, (1, self.estimator.item_biases.shape[0])
        )
        fx += np.tile(
            self.estimator.item_biases, (1, self.estimator.user_biases.shape[0])
        ).T
