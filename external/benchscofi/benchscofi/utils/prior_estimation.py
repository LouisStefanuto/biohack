...
def generate_Censoring_dataset(
    pi=0.3, c=0.3, N=100, nfeatures=50, mean=0.5, std=1, exact=True, random_state=1234
):
    r"""
    ...
    # Input to stanscofi
    user_list, item_list, feature_list = [
        list(map(str, x)) for x in [range(Nsqrt), range(Nsqrt), range(nfeatures // 2)]
    ]
    ratings_mat = pd.DataFrame(ratings_mat, columns=user_list, index=item_list).astype(
        int
    )
    labels_mat = pd.DataFrame(labels_mat, columns=user_list, index=item_list).astype(
        int
    )
...
def generate_CaseControl_dataset(
    N=100,
    nfeatures=50,
    pi=0.3,
    sparsity=0.01,
...
def roc_aided_estimation(
    scores_all, true_all, regression_type=[1, 2][0], show_plot=False, verbose=False
):
    r"""
    Class prior estimation under the case-control setting, that is,
    ...
    def binomial_deviance(x):
        ...
        if regression_type == 1:
            ...
            def f(alpha):
                val = (1 - gamma) * power(
                    1 + Delta * (1 / power(alpha, mu) - 1), -1 / mu
                ) + gamma * alpha
                return val

        dev = -2 * np.sum(
            np.multiply(mean_tprs, log(f(base_fpr)))
            + np.multiply(1 - mean_tprs, log(1 - f(base_fpr)))
        )
    ...
    x0 = np.array(
        [1] * (regression_type + 1)
    )  # gamma, Delta(, mu if regression_type=2)
    ...
    res = minimize(
        ...
        bounds=bnds,
        method="nelder-mead",
        options={"xatol": 1e-8, "disp": verbose, "maxiter": 1000},
    )
    args = res.x.tolist()
    if regression_type == 1:
        return args[0]
...
def divergence_aided_estimation(
    X,
    y,
    lmb=1,
    sigma=1.0,
    ...
    if divergence_type == "L1-distance":
        ...
        def approx_div(pi):
            betas = [
                pi / np.sum(pos_x) * basis_mat[l, pos_x].sum()
                if (pos_x.sum() > 0)
                else 0
                for l in range(basis_mat.shape[0])
            ]
            betas = [
                betas[l]
                - (
                    1 / np.sum(unl_x) * basis_mat[l, unl_x].sum()
                    if (unl_x.sum() > 0)
                    else 0
                )
                for l in range(basis_mat.shape[0])
        ...
        R = np.concatenate(
            (np.column_stack((0, R1)), np.column_stack((R1.T, R3))), axis=0
        )
        H = np.array([
            1
            / np.sum(x)
            * np.array(
                ...
                + [
                    np.sum([
                        basis_mat[l, i] for i in range(basis_mat.shape[0]) if (x[i])
                    ])
                    for l in range(basis_mat.shape[0])
                ]
            ).T
            for x in [unl_x, pos_x]
        ]).T
        basis_mat = np.concatenate(
            (np.ones((1, basis_mat.shape[1])), basis_mat), axis=0
        )
        basis_mat = np.concatenate(
            (np.ones((1, basis_mat.shape[0])).T, basis_mat), axis=1
        )
        G = (1 / basis_mat.shape[0]) * np.sum(
            [
                basis_mat[l, :].reshape((1, -1)).T.dot(basis_mat[l, :].reshape((1, -1)))
                for l in range(basis_mat.shape[0])
            ],
            axis=1,
        )
        ...
        def approx_div(pi):
            ...
            return (
                -0.5 * theta.dot(H.T).dot(GlR).dot(G).dot(GlR).dot(H.dot(theta.T))
                + theta.dot(H.T).dot(GlR).dot(H.dot(theta.T))
                - 0.5
            )

    if show_plot:
        plt.figure(figsize=(2, 2))
        pi_ls = [0.1 * p for p in range(10)]
