...
class DRRS(BasicModel):
    ...
    def model_fit(self, X_s, X_p, A_sp):
        ...
        drugsim, diseasesim, didra = [
            x + ".txt" for x in ["DrugSim", "DiseaseSim", "DiDrA"]
        ]
        for x, tx in zip([X_s, X_p, A_sp], [drugsim, diseasesim, didra], strict=False):
            pd.DataFrame(x, index=range(x.shape[0]), columns=range(x.shape[1])).to_csv(
                filefolder + "/" + tx, sep="\t", header=None, index=None
            )
        os.environ["LD_LIBRARY_PATH"] = (
            "%s/v80/runtime/glnxa64:%s/v80/bin/glnxa64:%s/v80/sys/java/jre/glnxa64/jre/lib/amd64/server:%s/v80/sys/os/glnxa64:%s/v80/sys/java/jre/glnxa64/jre/lib/amd64:%s/v80/sys/java/jre/glnxa64/jre/lib/amd64/native_threads"
            % tuple([self.MCR_HOME] * 6)
        )
        ...
        self.predictions = np.loadtxt(
            "%s/Result_dr_Mat.txt" % filefolder, delimiter="\t"
        ).T
