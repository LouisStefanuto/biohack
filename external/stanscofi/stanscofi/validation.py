...
Fscore = lambda y_true, y_pred, u, beta: fbeta_score(
...
PrecisionK = lambda y_true, y_pred, k, u1: precision_at_k(
    y_true[np.argsort(-y_pred)], k
)
RecallK = lambda y_true, y_pred, k, u1: recall_at_k(
    y_true[np.argsort(-y_pred)], np.sum(y_true > 0), k=k
)
...
MAP = lambda y_true, y_pred, u, u1: mean_average_precision([
...
def compute_metrics(scores, predictions, dataset, metrics, k=1, beta=1, verbose=False):
    ...
    if verbose and n_ignored > 0:
        print(
            "<validation.compute_metrics> Computed on #users=%d, %d ignored (%2.f perc)"
            % (len(user_ids), n_ignored, 100 * n_ignored / len(user_ids))
        )
    if len(aucs) == 0 or len(fscores) == 0:
        metrics = pd.DataFrame(
            [], index=metrics, columns=["Average", "StandardDeviation"]
        )
        return metrics, {}
    metrics = pd.DataFrame(
        [[f(metrics_list[m]) for f in [np.mean, np.std]] for m in metrics_list],
        index=metrics,
    ...
    metrics = pd.concat(
        (
            metrics,
            pd.DataFrame(
                [[k, beta]], index=["arguments (k, beta)"], columns=metrics.columns
            ),
        ),
        axis=0,
    )
...
def plot_metrics(
    ...
    if len(aucs) > 0:
        ...
        print(
            "<validation.plot_metrics> Can't plot: only 1 relevance level in true scores"
        )
        return
    ...
    axes[0, 0].plot(
        ...
        "b",
        alpha=0.8,
        label=model_name + " (AUC = %0.2f %s %0.2f)" % (auc, "$\\pm$", std_auc),
    )
    axes[0, 0].fill_between(base_fpr, tprs_lower, tprs_upper, color="blue", alpha=0.2)
    axes[0, 0].plot(
        [0, 1], [0, 1], linestyle="--", lw=2, color="r", alpha=0.8, label="Constant"
    )
    ...
    axes[0, 1].plot(
        ...
        "b",
        alpha=0.8,
        label=model_name + " (F = %0.2f %s %0.2f)" % (fs, "$\\pm$", std_fs),
    )
    axes[0, 1].fill_between(base_pres, recs_lower, recs_upper, color="blue", alpha=0.2)
    axes[0, 1].plot(
        [0, 1], [1, 0], linestyle="--", lw=2, color="r", alpha=0.8, label="Constant"
    )
    ...
    labels, counts = np.unique(
        np.multiply(predictions, ground_truth) + 2 * ground_truth, return_counts=True
    )
    ...
    h = axes[1, 1].pie(
        counts[ids],
        labels=[
            {3: "TP", -3: "FP", 0: "UN", 1: "FN", -1: "TN"}[x] for x in labels[ids]
        ],
        autopct="%1.1f%%",
        colors=[
            {3: "green", -3: "red", 0: "white", 1: "darkred", -1: "darkgreen"}[x]
            for x in labels[ids]
        ],
    )
    ...
    axes[1, 1].text(
        -1.0,
        -1.25,
        "%d datapoints (%d known matchings)\nAccuracy=%.2f on known matchings"
        % (np.sum(counts), np.sum(counts[ids]), accuracy),
    )
    # Show
    plt.show()
