...
class BasicModel:
    ...
    def predict_proba(self, test_dataset, default_zero_val=1e-31):
        ...
        """
        scores = self.model_predict_proba(
            *self.preprocessing(test_dataset, is_training=False)
        )
        if (scores != 0).any():
        ...
        if scores.shape == test_dataset.folds.shape:
            scores[(scores == 0) & (test_dataset.folds.toarray() == 1)] = (
                default_val  # avoid removing these zeroes
            )
        ...
        scores[(scores == 0) & (test_dataset.folds.data == 1)] = (
            default_val  # avoid removing these zeroes
        )
        scores_arr = coo_array(
            (scores, (test_dataset.folds.row, test_dataset.folds.col)),
    ...
    def predict(self, scores, threshold=0.5):
        ...
        preds = coo_array(
            (scores.toarray() != 0).astype(int)
            * ((-1) ** (scores.toarray() <= threshold))
        )
    ...
    def recommend_k_pairs(self, dataset, k=1, threshold=None):
        ...
        if k is not None:
            ...
            ids_list = np.dstack(
                np.unravel_index(
                    (-vals.toarray()).ravel().argsort(), scores.toarray().shape
                )
            )[0][:k, :].tolist()
        candidates = [
            [dataset.item_list[i], dataset.user_list[j], scores.toarray()[i, j]]
            for i, j in ids_list
        ]
        return candidates
    ...
    def print_scores(self, scores):
        ...
        print("* Scores")
        print(
            "%d unique items, %d unique users"
            % (len(np.unique(scores.row)), len(np.unique(scores.col)))
        )
        print(
            "Scores: Min: %f\tMean: %f\tMedian: %f\tMax: %f\tStd: %f\n"
            % tuple([
                f(scores.data) for f in [np.min, np.mean, np.median, np.max, np.std]
            ])
        )
    ...
    def print_classification(self, predictions):
        ...
        print(
            "%d unique items, %d unique users"
            % (len(np.unique(predictions.row)), len(np.unique(predictions.col)))
        )
        print(
            "Positive class: %d, Negative class: %d\n"
            % (
                (csr_array(predictions) == 1).sum(),
                (csr_array(predictions) == -1).sum(),
            )
        )
...
class NMF(BasicModel):
    ...
    def preprocessing(self, dataset, is_training=True):
        ...
        """
        mat = np.nan_to_num(dataset.ratings.toarray(), copy=True, nan=0.0)
        A = mat - np.min(mat)
        return [A.T]
...
class LogisticRegression(BasicModel):
    ...
    def __init__(self, params):
        ...
        assert self.preprocessing_str in [
            "Perlman_procedure",
        ...
        self.estimator = Logit(**{
            p: params[p] for p in params if (p not in ["preprocessing_str", "subset"])
        })
    ...
    def preprocessing(self, dataset, is_training=True):
        ...
        """
        X, y, scalerS, scalerP, filter_ = preprocessing_XY(
            dataset,
            self.preprocessing_str,
            subset_=self.subset,
            filter_=self.filter,
            scalerS=self.scalerS,
            scalerP=self.scalerP,
            inf=2,
            njobs=1,
        )
