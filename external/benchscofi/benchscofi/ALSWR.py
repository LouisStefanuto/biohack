...
from .implementations import AlternatingLeastSquares
...
class ALSWR(BasicModel):
    def __init__(self, params=None):
        ...
        self.model = AlternatingLeastSquares.ALSWR(**{
            (k if (k != "random_state") else "seed"): params[k] for k in params
        })
    ...
    def preprocessing(self, dataset, is_training=True):
        ...
        if is_training:
            ratings = csr_matrix(
                (dataset.ratings.data, (dataset.ratings.col, dataset.ratings.row)),
                shape=dataset.ratings.T.shape,
            )
        else:
            ids = np.argwhere(np.ones(dataset.ratings.shape))
            ratings = csr_matrix(
                (
                    dataset.ratings.toarray().ravel(),
                    (ids[:, 1].ravel(), ids[:, 0].ravel()),
                ),
                shape=dataset.ratings.T.shape,
            )
        return [ratings]
