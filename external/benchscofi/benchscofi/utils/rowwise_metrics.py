...
def calc_mpr_auc(scores, dataset, transpose=False):
    ...
    return (
        all_u_mpr,
        all_u_auc,
        np.sum(all_u_mpr) / total_pos,
        np.sum(all_u_auc) / total_m,
    )
...
def calc_auc(scores, dataset, transpose=False, verbose=False):
    user_ids = (
        np.unique(dataset.folds.col) if (transpose) else np.unique(dataset.folds.row)
    )
    n_ignored = 0
    ...
    for user_id in user_ids:
        user_ids_i = (
            np.argwhere(dataset.folds.col == user_id)
            if (transpose)
            else np.argwhere(dataset.folds.row == user_id)
        )
        if len(user_ids_i) == 0:
        ...
        if (len(np.unique(user_truth)) == 2) and (1 in user_truth):
            ...
            if omegaplus * omegaminus == 0:
                ...
                continue
            num = sum([
                int(user_pred[j] > user_pred[jp])
                for j in np.argwhere(user_truth == 1)
                for jp in np.argwhere(user_truth < 1)
            ])
    ...
    if verbose:
        print(
            "#ignored users = %d (%d perc.)"
            % (n_ignored, n_ignored * 100 / len(user_ids))
        )
    return rowwise_aucs
