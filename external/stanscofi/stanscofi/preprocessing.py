...
def preprocessing_XY(
    ...
    """
    assert njobs > 0
    assert preprocessing_str in [
        "Perlman_procedure",
        "meanimputation_standardize",
        "same_feature_preprocessing",
    ]
    if preprocessing_str == "Perlman_procedure":
        X, y = eval(preprocessing_str)(
            dataset, njobs=njobs, sep_feature=sep_feature, missing=-666, verbose=False
        )
        scalerS, scalerP = None, None
    if preprocessing_str == "meanimputation_standardize":
        X, y, scalerS, scalerP = eval(preprocessing_str)(
            dataset,
            subset=subset_,
            scalerS=scalerS,
            scalerP=scalerP,
        ...
        if (subset_ is not None) or (filter_ is not None):
            if (subset_ is not None) and (filter_ is None):
                with np.errstate(over="ignore"):
                    x_vars = [
                        np.nanvar(X[:, i]) if (np.sum(~np.isnan(X[:, i])) > 0) else 0
                        for i in range(X.shape[1])
                    ]
                    x_vars = [
                        x if (not np.isnan(x) and not np.isinf(x)) else 0
                        for x in x_vars
                    ]
...
class CustomScaler:
    ...
    def fit_transform(self, mat, subset=None, verbose=False):  # elements x features
        ...
        """
        mat_nan = np.nan_to_num(
            mat, copy=True, nan=np.nan, posinf=self.posinf, neginf=self.neginf
        )
        assert mat_nan.shape == mat.shape
        if (subset is not None) or (self.filter is not None):
            if verbose:
                print(
                    "<preprocessing.CustomScaler> Selecting the %d most variable features out of %d..."
                    % (subset, mat_nan.shape[1])
                )
            if (subset is not None) and (self.filter is None):
                with np.errstate(over="ignore"):
                    x_vars = [
                        np.nanvar(mat_nan[:, i])
                        if (np.sum(~np.isnan(mat_nan[:, i])) > 0)
                        else 0
                        for i in range(mat_nan.shape[1])
                    ]
                    x_vars = [
                        x if (not np.isnan(x) and not np.isinf(x)) else 0
                        for x in x_vars
        ...
        assert (
            mat_nan.shape[1]
            == (mat.shape[1] if (self.filter is None) else len(self.filter))
            and mat_nan.shape[0] == mat.shape[0]
        )
        mat_nan[:, np.sum(~np.isnan(mat_nan), axis=0) == 0] = (
            0  # avoid overflow warning from SimpleImputer
        )
        if verbose:
            print(
                "<preprocessing.CustomScaler> %d perc. of missing values (||.|| = %f)..."
                % (
                    100 * np.sum(np.isnan(mat_nan)) / np.prod(list(mat_nan.shape)),
                    np.linalg.norm(mat_nan),
                ),
                end=" ",
            )
        ...
        elif np.sum(np.isnan(mat_nan)) > 0:
        ...
        assert (
            mat_nan_std.shape[1]
            == (mat.shape[1] if (self.filter is None) else len(self.filter))
            and mat_nan_std.shape[0] == mat.shape[0]
        )
        return mat_nan_std


def meanimputation_standardize(
    dataset, subset=None, scalerS=None, scalerP=None, inf=int(1e1), verbose=False
):
    """
    ...
    if verbose:
        print(
            "<preprocessing.meanimputation_standardize> Preprocessing of item feature matrix..."
        )
    S_ = scalerS.fit_transform(
        dataset.items.toarray().T, subset=subset, verbose=verbose
    )  # items x features=subset
    if scalerP is None:
        scalerP = CustomScaler(posinf=inf, neginf=-inf)
    if verbose:
        print(
            "<preprocessing.meanimputation_standardize> Preprocessing of user feature matrix..."
        )
    P_ = scalerP.fit_transform(
        dataset.users.toarray().T, subset=subset, verbose=verbose
    )  # users x features=subset
    X = np.column_stack((
        S_[dataset.folds.row, :],
        P_[dataset.folds.col, :],
    ))  # (item, user) pairs x (item + user features)
...
def same_feature_preprocessing(dataset, operator):
    ...
    y = dataset.ratings.toarray()[dataset.folds.row, dataset.folds.col].ravel()
    S_, P_ = [
        np.nan_to_num(x.toarray().T, copy=True, nan=0.0)
        for x in [dataset.items, dataset.users]
    ]
...
def Perlman_procedure(
    dataset, njobs=1, sep_feature=None, missing=-666, inf=2, verbose=False
):
    """
    ...
    if sep_feature is not None:
        ...
        is_user, is_item = 0, 0
    if (is_user not in [0, dataset.users.shape[0]]) or (
        is_item not in [0, dataset.items.shape[0]]
    ):
        print(
            'Ensure that sep_feature(="%s") is set to the correct value. If there is only one type of feature, set this parameter to None.'
            % str(sep_feature)
        )
    ...
    y = dataset.ratings.toarray()[dataset.folds.row, dataset.folds.col].ravel()
    P, S = [
        np.nan_to_num(x.toarray(), copy=True, nan=0, posinf=inf, neginf=-inf)
        for x in [dataset.users, dataset.items]
    ]
    ...
    nuser_features = np.array(
        [missing] * len(dataset.user_features)
        if (is_user == 0)
        else [x.split(sep_feature)[0] for x in dataset.user_features]
    )
    nitem_features = np.array(
        [missing] * len(dataset.item_features)
        if (is_item == 0)
        else [x.split(sep_feature)[0] for x in dataset.item_features]
    )
    ...
    nuser_nms = np.array([
        dataset.user_list.index(x) if (x in dataset.user_list) else missing
        for x in (
            dataset.user_features
            if (is_user == 0)
            else [y.split(sep_feature)[1] for y in dataset.user_features]
        )
    ])
    nitem_nms = np.array([
        dataset.item_list.index(x) if (x in dataset.item_list) else missing
        for x in (
            dataset.item_features
            if (is_item == 0)
            else [y.split(sep_feature)[1] for y in dataset.item_features]
        )
    ])
    Nuf, Nif = len(set(nuser_features)), len(set(nitem_features))
    if verbose:
        print(
            "<preprocessing.Perlman_procedure> %d item similarities, %d user similarities"
            % (Nuf, Nif)
        )
    item_feature_mask = {
        fi: (nitem_features == fi).astype(int) for fi in set(nitem_features)
    }
    item_mask = [S[:, i] * (nitem_nms != i).astype(int) for i in range(dataset.nitems)]
    user_feature_mask = {
        fu: (nuser_features == fu).astype(int) for fu in set(nuser_features)
    }
    user_mask = [P[:, u] * (nuser_nms != u).astype(int) for u in range(dataset.nusers)]
    known = np.array([
        0
        if ((nitem_nms[i] == missing) or (nuser_nms[u] == missing))
        else int(dataset.ratings.toarray()[nitem_nms[i], nuser_nms[u]] != 0)
    ...
    if verbose:
        print(
            "<preprocessing.Perlman_procedure> For %d ratings pairs, identified %d features"
            % (ids.shape[0], Nuf * Nif)
        )
    features = [[a, b] for a in set(nitem_features) for b in set(nuser_features)]

    def single_run(
        fi, fu, known, item_feature_mask, item_mask, user_feature_mask, user_mask, ids
    ):
        if verbose:
            print(
                "<preprocessing.Perlman_procedure> Considering item feature '%s' and user feature '%s' (%d jobs)"
                % (
                    str(fi) if (fi != missing) else "{1}",
                    str(fu) if (fu != missing) else "{1}",
                    njobs,
                )
            )
    ...
    if njobs == 1:
        vals = [
            ...
            for [fi, fu] in tqdm(features)
        ]
    else:
        vals = Parallel(n_jobs=min(njobs, len(features)), backend="loky")(
            delayed(single_run)(
                fi,
                fu,
                known,
